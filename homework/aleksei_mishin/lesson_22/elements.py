...
class BaseElement:
    ...
    def click_on_random_element(self):
        with allure.step(f"Клик по рандомному элементу из {self.name}"):
            element = random.choice(self.get_elements())
            element.click()

    def scroll_to_element(self):
        self.browser.execute_script("arguments[0].scrollIntoView(true);", self.get_element())
